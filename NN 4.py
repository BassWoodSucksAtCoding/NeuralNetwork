...
class NeuralNetwork(nn.Module):
    def __init__(self):
        ...
        self.linear_relu_stack = nn.Sequential(
            nn.Linear(28*28, 512), # Converts the 784 elements each image has (each pixel of the image is an element) into 512 new outputs
            nn.ReLU(), # Adds complexities
            nn.Linear(512,512), # Converts the 512 features into 512 new features
            nn.ReLU(),
            nn.Linear(512,512),
            nn.ReLU(),
            nn.Linear(512,512),
            nn.ReLU(),
            nn.Linear(512,10) # Eventually makes 10 outputs since there are 10 digits
        )
    
    # What to do upon getting an input
    def forward(self, x):
        x = self.flatten(x) # Flattening turns (1,28,28) tensor into 1d 784 which can be processed by the Linear layer
...
if train_model:
    ...
    with torch.no_grad():
        ...
        # Getting the predictions for the image
        pred = model(image)

        # Getting the digit it is most likely to be (fetching the largest percentage)
